...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        if cross_attention_dim is not None:
            self.attn2 = CrossAttentionModified(
                query_dim=dim,
                cross_attention_dim=cross_attention_dim,
                heads=num_attention_heads,
                dim_head=attention_head_dim,
    ...
    def set_use_memory_efficient_attention_xformers(self, use_memory_efficient_attention_xformers: bool, attention_op: Optional[Callable] = None):
        if not is_xformers_available():
            ...
            if self.attn2 is not None:
                ...
                # self.attn2.set_processor(XFormersAttnProcessor2)
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None, \
        ...
        if self.only_cross_attention:
            hidden_states = (
                    self.attn1(norm_hidden_states, encoder_hidden_states, attention_mask=attention_mask, inter_frame=inter_frame, **kwargs)[0] + hidden_states
            )
        else:
            hidden_states = self.attn1(norm_hidden_states, attention_mask=attention_mask, video_length=video_length, inter_frame=inter_frame, **kwargs)[0] + hidden_states
        ...
        if self.attn2 is not None:
            ...
            hidden_states = (
                    self.attn2(
                        norm_hidden_states, encoder_hidden_states=encoder_hidden_states, attention_mask=attention_mask, **kwargs
                    )[0]
                    + hidden_states
            )
...
class XFormersAttnProcessor2(XFormersAttnProcessor):
    def __call__(
            self,
            attn: Attention,
        ...
        return hidden_states, query, key


class CrossAttentionModified(Attention):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
        if self.processor.__class__.__name__ == 'XFormersAttnProcessor':
            self.set_processor(XFormersAttnProcessor2())
        if self.inject_q is not None:
            cross_attention_kwargs['inject_q'] = self.inject_q
        if self.inject_k is not None:
            cross_attention_kwargs['inject_k'] = self.inject_k

        hidden_states, query, key = self.processor(
            self,
            hidden_states,
            encoder_hidden_states=encoder_hidden_states,
...
class FullyFrameAttention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, inter_frame=True, **kwargs):
        ...
        return hidden_states, query1, key1